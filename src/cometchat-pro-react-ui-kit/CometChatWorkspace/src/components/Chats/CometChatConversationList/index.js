...
import { CometChatConfirmDialog, CometChatToastNotification, SidebarMore } from "../../Shared";
...
class CometChatConversationList extends React.Component {
	...
	setSelectedConversation = () => {
		...
		conversationList.forEach(conversation => {
			if (conversation?.conversationType === this.getContext().type) {
				...
				if ((conversation?.conversationType === CometChat.ACTION_TYPE.TYPE_GROUP && conversation?.conversationWith?.guid === this.getContext().item?.guid)) {
					this.selectedConversation = { ...conversation };
				}
			}
		});
	...
	onDeleteConfirm = e => {
		...
		if (optionSelected === "yes") {
			...
			CometChat.deleteConversation(conversationWith, conversation.conversationType)
				.then(deletedConversation => this.conversationDeleted(conversation))
	...
	toggleChannel = flag => {
		this.setState({ showChannel: !this.state.showChannel})
	}

	showThreadsSideBar = falg => {
		this.setState(
		{
			showThreads: !this.state.showThreads,
			showMoreModal: !this.state.showMoreModal,
		})
	}

	showReactionSideBar = falg => {
		this.setState(
		{
			showReaction: !this.state.showReaction,
			showMoreModal: !this.state.showMoreModal,
		})
	}

	showSavedItemSideBar = falg => {
		this.setState(
		{
			showSavedItem: !this.state.showSavedItem,
			showMoreModal: !this.state.showMoreModal,
		})
	}

	showAllUnReadSideBar = falg => {
		this.setState(
		{
			showAllUnRead: !this.state.showAllUnRead,
			showMoreModal: !this.state.showMoreModal,
		})
	}

	showAllDmsSideBar = falg => {
		this.setState(
		{
			showAllDms: !this.state.showAllDms,
			showMoreModal: !this.state.showMoreModal,
		})
	}
	...
	render() {
		...
		const conversationList = this.state.conversationlist.map((conversation, key) => {
			...
			if (this.getContext() && Object.keys(this.getContext().item).length && this.getContext().type.trim().length) {
				...
				// 	this.selectedConversation = conversation;
				// } else 
				if (this.getContext().type === CometChat.ACTION_TYPE.TYPE_GROUP && this.getContext().item.guid === conversation.conversationWith.guid) {
					this.selectedConversation = conversation;
				}
			}
			if(conversation.conversationType == "group") {
				return <CometChatConversationListItem 
					...
					showmessage={this.props.messageListShow}
				/>;
			}
		});
		
		// user list
		const UserList = this.state.conversationlist.map((conversation, key) => {
			let selectedConversation = null;
			if (this.getContext() && Object.keys(this.getContext().item).length && this.getContext().type.trim().length) {
				// if (this.getContext().type === CometChat.ACTION_TYPE.TYPE_USER && this.getContext().item.uid === conversation.conversationWith.uid) {
				// 	this.selectedConversation = conversation;
		...
		let showMoreDialog = null;
		if (this.state.showMoreModal) {
			showMoreDialog = (
				<SidebarMore
					showThreads 	= {this.showThreadsSideBar}
					showReaction 	= {this.showReactionSideBar}
					showSavedItem 	= {this.showSavedItemSideBar}
					showAllUnRead 	= {this.showAllUnReadSideBar}
					showAllDms 		= {this.showAllDmsSideBar}
				/>
			);
		}
		...
		const chatList = (
			<div css={chatsWrapperStyle(this.props, theme)} className="chats">
					{closeBtn}
					<h4 css={TitleHeaderStyle(this.props)} className="header__title">
						{Translator.translate("Welcome", this.props.lang)}
					</h4>
					<div></div>

				{/* Threads */}
				{
					this.state.showThreads ? 
						<div css={chatsHeaderStyle(theme)} className="chats__header" onClick={this.props.threadMsgShow}>
				...
				{/* Saved items */}
				{
					this.state.showSavedItem ? 
						<div css={chatsHeaderStyle(theme)} className="chats__header" onClick={this.getSavedItems}>
							<TurnedInNot css={materialIcons_opt()} className="materialIcons" />Saved items
						</div> 
					: ''
				}
				...
				{/* More */}
				<div css={chatsHeaderStyleMore(theme)} className="chats__header" onClick={this.showMore}>
					<MoreVert css={materialIcons_opt()} className="materialIcons" />More
					{showMoreDialog}
				</div>

				{/* group list */}
				<div css={chatsHeaderStyle(theme)} className="chats__header" onClick={this.toggleChannel}>
				{
					this.state.showChannel ? <ArrowDropDown css={materialIcons()} className="materialIcons" /> : <ArrowRight css={materialIcons()} />
				}
					Channels
				</div>
				{messageContainer}
				<div css={chatsListStyle()} className="chats__list" onScroll={this.handleScroll} ref={el => (this.chatListRef = el)}>
					{ this.state.showChannel ? conversationList : [] }
				</div>

				{/* user list */}
				<div css={chatsHeaderStyle(theme)} className="chats__header" onClick={this.toggleUserList}>
				{
					this.state.showUsers ? <ArrowDropDown css={materialIcons()} className="materialIcons" /> : <ArrowRight css={materialIcons()} />
				}
					Direct messages
				</div>
				{messageContainer}
